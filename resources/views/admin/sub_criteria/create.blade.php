...
@section('title', 'Tambah Skala Konversi Nilai Rapor') {{-- Judul diubah --}}
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h5 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-plus-circle"></i> Tambah Skala Konversi Nilai Rapor
                </h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.subcriteria.store') }}" method="POST">
                    ...
                    <table class="table table-bordered mb-4 bg-light">
                        <tr>
                            ...
                            <td class="font-weight-bold">{{ $criteria->major->name ?? 'N/A' }}</td>
                        </tr>
                        <tr>
                            <th>Mata Pelajaran (Subject)</th>
                            <td class="font-weight-bold text-primary">{{ $criteria->subject->name ?? 'N/A' }}</td>
                        </tr>
                        <tr>
                            <th>Bobot</th>
                            <td>
                                <span class="font-weight-bold text-warning p-2">{{ $criteria->weight }}</span>
                            </td>
                        </tr>
                        <tr>
                            <th>Atribut</th>
                            <td>
                                <span class="font-weight-bold text-secondary p-2">{{ ucwords($criteria->attribute_type) }}</span>
                            </td>
                        </tr>
                    </table>
                    ...
                    <div class="form-group">
                        ...
                        <small class="form-text text-muted">Nilai ini adalah nilai $X_{ij}$ yang akan digunakan dalam
                            perhitungan MOORA (biasanya 1 sampai 5 atau 1 sampai 10).</small>
                        @error('value')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <p class="text-danger mt-1 mb-4"><i class="fas fa-exclamation-triangle"></i> Pastikan Rentang Nilai
                        Rapor (Minimum - Maksimum) tidak tumpang tindih dengan rentang lain untuk kriteria
                        ini.</p>
